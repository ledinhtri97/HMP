...
from utils.body_utils import mano_full_chain
from utils.torch_utils import copy2cpu as c2c
from utils.fitting_utils import perspective_projection


mano_connections = mano_full_chain(is_rhand=True)


# smpl_body_connections = [[11, 8], [8, 5], [5, 2], [2, 0], [10, 7], [7, 4], [4, 1], [1, 0], 
...
colors = {
    ...
    "maroon": np.array([128, 0, 0])/255,
    "pink": np.array([255, 200, 220])/255,
    "brown": np.array([170, 110, 40])/255,
    "orange": np.array([255, 150, 0])/255,
    "coral": np.array([255, 215, 180])/255,
    "olive": np.array([128, 128, 0])/255,
    "yellow": np.array([255, 235, 0])/255,
    "beige": np.array([255, 250, 200])/255,
    "lime": np.array([190, 255, 0])/255,
    "green": np.array([0, 190, 0])/255,
    "mint": np.array([162, 228, 184])/255,
    "teal": np.array([0, 128, 128])/255,
    "cyan": np.array([100, 255, 255])/255,
    "navy": np.array([0, 0, 128])/255,
    "blue": np.array([67, 133, 255])/255,
    "purple": np.array([130, 0, 150])/255,
    "lavender": np.array([230, 190, 255])/255,
    "magenta": np.array([255, 0, 255])/255,
    "gray": np.array([128, 128, 128])/255
}

def vis_training_curve():

    
    from torch.utils.tensorboard import SummaryWriter
    # writer = SummaryWriter()
    local_host_id = 8088; webbrowser.open_new_tab('http://localhost:' + str(local_host_id)); os.system("tensorboard --logdir=out/ --host localhost --port 8088")
...
def viz_joint_ids(img, joint3d):

    rgb_img = Image.fromarray(copy.deepcopy(img.astype(np.uint8)))
    draw = ImageDraw.Draw(rgb_img)

    # project 3d joints to 2d
    perspective_projection(points, rotation, translation, focal_length, camera_center)
...
def viz_smpl_seq(body, imw=1080, imh=1080, fps=30, contacts=None,
                ...
                skel_connections=mano_connections, img_extn='png', ground_alpha=1.0, body_alpha=None, mask_seq=None,
                cam_offset=[0.0, 4.0, 1.25], ground_color0=[0.8, 0.9, 0.9], ground_color1=[0.6, 0.7, 0.7], mesh_color=colors['coral'],
                skel_color=colors["red"],
                joint_rad=0.005,
    ...
    if render_body or vtx_list is not None:
        ...
        vertex_colors = np.tile(mesh_color, (nv, 1))
...
def viz_results(body_pred, body_gt, fps, viz_out_dir=None, base_name=None, contacts=None, pred_joints=None, gt_joints=None,
                ...
                skel_connections=mano_connections, ground_alpha=1.0, body_alpha=None, point_color=[0.0, 1.0, 0.0],
...
# from vis.mesh_viewer import MeshViewer